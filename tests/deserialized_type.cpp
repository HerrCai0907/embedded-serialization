...
TEST(deserialized_type, tuple) {
  ...
  using Expect = std::tuple<u8, uint16_t, uint32_t, SerializedSpan<const u8, 1U, 1U>>;
  EXPECT_EQ(typeid(DeserializedType<Input>::type), typeid(Expect));
}
...
TEST(deserialized_type, dynamic_array_tuple) {
  ...
  using Expect = std::tuple<SerializedArray<std::tuple<u8, uint16_t, uint32_t, SerializedSpan<const u8, 1U, 1U>>, 2U>>;
  EXPECT_EQ(typeid(DeserializedType<Input>::type), typeid(Expect));
}
