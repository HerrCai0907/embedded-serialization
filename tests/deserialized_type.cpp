...
TEST(deserialized_type, dynamic_array_dynamic_array) {
  ...
  using Expect = std::tuple<std::array<SerializedSpan<const u8, 1U, 2U>, 4U>>;
  EXPECT_EQ(typeid(DeserializedType<Input>::type), typeid(Expect));
}
TEST(deserialized_type, dynamic_array_dynamic_array_dynamic_array) {
  ...
  using Expect = std::tuple<std::array<std::array<SerializedSpan<const u8, 1U, 2U>, 4U>, 6U>>;
  EXPECT_EQ(typeid(DeserializedType<Input>::type), typeid(Expect));
}
...
TEST(deserialized_type, dynamic_array_static_array) {
  ...
  using Expect = std::tuple<std::array<SerializedSpan<const u8, 1U, 1U>, 3U>>;
  EXPECT_EQ(typeid(DeserializedType<Input>::type), typeid(Expect));
}
...
TEST(deserialized_type, dynamic_array_tuple) {
  ...
  using Expect = std::tuple<std::array<std::tuple<u8, uint16_t, uint32_t, SerializedSpan<const u8, 1U, 1U>>, 2U>>;
  EXPECT_EQ(typeid(DeserializedType<Input>::type), typeid(Expect));
}